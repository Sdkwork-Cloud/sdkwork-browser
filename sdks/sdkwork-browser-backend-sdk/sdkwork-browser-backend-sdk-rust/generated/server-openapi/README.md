# sdkwork-browser-backend-sdk (Rust)
...
```bash
cargo add sdkwork-browser-backend-sdk
```

## Quick Start

```rust
use sdkwork_browser_backend_sdk::{SdkworkBackendClient, SdkworkConfig};
...
client.set_access_token("your-access-token");
    ...
    let result = client.browser().engines_list().await?;
    println!("{result:?}");
    Ok(())
}
```
...
let outcome: Result<(), _> = async {
    client.browser().engines_list().await?;
    Ok(())
}.await;
