...
```bash
cargo add sdkwork-browser-app-sdk
```

## Quick Start

```rust
use sdkwork_browser_app_sdk::{SdkworkAppClient, SdkworkConfig};
...
client.set_access_token("your-access-token");
    ...
    let result = client.browser().ai_actions_create(&body).await?;
    println!("{result:?}");
    Ok(())
}
```
...
let outcome: Result<(), _> = async {
    let mut body = HashMap::new();
    client.browser().ai_actions_create(&body).await?;
    Ok(())
}.await;
